...
import { ChatPolicyError, findOrCreateDirectConversation, listConversationsForUser } from '@/lib/chat/service'
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const message = error instanceof ChatPolicyError ? error.message : 'No se pudo crear la conversación'
    const status = error instanceof ChatPolicyError ? 403 : 400
